...
const httpHandle = async function(_this){
  ...
  //检查黑名单列表
  let arr = [];
  if(_this.headers['x-forwarded-for']){
    arr.push({
  ...
  if(blacklistRes.length > 0){
    return msg = blacklistRes[0].content;
  }
  //token检查
